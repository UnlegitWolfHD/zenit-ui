...
- **Host bindings and caller attributes.** `'[attr.x]': 'y() ? "…" : null'` does not leave the attribute alone when the expression is `null`: it **removes** it, on every change detection run, and thereby deletes what the caller wrote there, a static attribute as well as a binding of its own. So a host binding may own only an attribute no caller would write. For everything a caller plausibly writes (`role`, `aria-*`, `tabindex`, `title`, `id`, `name`, `type`) pick one of three. A value the library alone decides is **borrowed**: `leiheAttribut('role', () => …)` from `lib/a11y/host-attribute.ts` writes only while the library has a value and gives back the caller's **latest** value, which is the rule "the input wins while it is set, the caller's attribute stands while it is not" (`z-segment`, `z-spinner`, `z-table-container`, `aria-disabled` on `button[zBtn]` and `a[zBtn]`). Latest, not the value at borrow time: a binding of the caller keeps writing while the library holds the attribute, so a `MutationObserver` runs for as long as it is borrowed, notes each new value as the one to give back and asserts the library value again. Do not decide such a thing from `HostAttributeToken` alone — that reads the static attribute at construction, when a binding of the caller has written nothing yet. An attribute that is a **list of tokens**, `aria-describedby` above all, gets `ZTokenAttribut` from the same file: it adds and removes exactly one token, keeps a fixed end of the list so two writers on the same element settle, and watches the attribute with a `MutationObserver` while its token stands, because a binding of the caller rewrites the whole attribute (`z-field` through `input[zInput]` and `z-select`, `zTooltip`). At most one writer per attribute **and end** on one element: two of them on the same end move the token past each other forever, which is a frozen tab, and a second claim therefore throws in development. Writing is skipped when nothing changes and the record of an own write is dropped with `takeRecords()` — Chromium queues a mutation record for a `setAttribute` with an unchanged value too, and answering that record with another write freezes the tab. And a **static** attribute that only has to be read once is `inject(new HostAttributeToken('type'), { optional: true })` (`button[zGameTile]`). Both writers create their observer lazily and only where there is one, because a component that writes such an attribute during the first change detection run would otherwise throw `MutationObserver is not defined` on the server. New blocks state in their guide which of the three applies, and the case gets two unit tests: one with the attribute written statically in the host template, one with `[attr.…]` bound to a signal that changes while the library holds the attribute.
- **Browser globals.** `MutationObserver`, `ResizeObserver` and `IntersectionObserver` do not exist on the server, and a constructor, a field initialiser, an `ngOnInit` or an effect of the first change detection run is reached there: constructing one in any of them throws `ReferenceError: X is not defined` and the consumer's page 404s instead of rendering. Create them lazily behind `typeof X === 'undefined' ? undefined : new X(…)` and call through `?.` from then on, or construct them inside `afterNextRender`, which never runs on the server. The first is the way when what the observer guards has to stand in the server HTML anyway — a locked `<a zBtn>` gets its `tabindex="-1"` there and only the re-assertion waits for the browser — the second when the whole job is a browser job, a measurement or projected text that can only be read after a render. The same holds for `window`, `localStorage` and `matchMedia`. **`projects/zenit-ui/src/lib/ssr.spec.ts` gets a case for every new observer**: it renders with all three globals stubbed away and asserts that nothing throws and that the attributes the library owns are in the HTML. It runs in jsdom, so it is not SSR; the real prerender gate is `npm run check:ssr`.
- **What the server withholds beyond the globals.** Its elements exist and answer `setAttribute`, `getAttribute` and `style`, but they have no layout and parse no values: `getBoundingClientRect` and `valueAsNumber` are `undefined` there, not zero and not `NaN`, and `document.defaultView` exists but has no `requestAnimationFrame`. So `!Number.isNaN(x)` is the wrong guard for anything read off an element — it says `true` for `undefined` and lets the missing value through; ask `Number.isFinite(x)` instead. That is what cost `z-slider` its value: it reported `undefined` to the form, the key fell out of the model, and `[formField]` failed with `this.field(...) is not a function`, swallowed by the error handler while the route still counted as prerendered. Which paths the server reaches is the whole question: a constructor, a field initialiser, an `effect` and **every destroy path** run there, while `afterNextRender` and `afterRenderEffect` never do. A destroy is the one that is easy to miss, because it runs after the last render on the server too — `ZStickyBar` measured the bars that were left there and took `/konfigurator` down. Measure in the next frame instead, which the server never reaches. Every DOM read on one of those paths gets a case in `ssr.spec.ts` that stubs away exactly what the server lacks.
