...
@Component({
  imports: [ZCheckbox, FormField],
  template: `<z-checkbox [formField]="formular.agb" ariaDescribedby="agb-fehler"
    >Ich stimme den Bedingungen zu</z-checkbox
  >`,
...
describe('ZCheckbox with Signal Forms', () => {
  function erzeuge() {
  ...
  it('takes required from the schema onto the native input', () => {
    const { feld } = erzeuge();
  ...
  it('puts ariaDescribedby onto the native input', () => {
    const { feld } = erzeuge();
