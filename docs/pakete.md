...
1. **Styles.** `spec/components/bundle.css` ist die Referenz. Jeder Umsetzer übernimmt die Abschnitte seiner Bausteine wörtlich in seine Style-Datei. Literale Werte, die dort stehen (Schriftgrößen 12 bis 56px, 1px/2px-Linien, 6px-Punkt, 16px-Spinner usw.), sind damit abgenommen. Jeder neue Wert außerhalb der Tokens ist ein Fund, außer er ist im Abschlussbericht begründet. Zwei dokumentierte Abweichungen von bundle.css: die Basisregel für font/color nutzt `:where()` (Spezifitätsfehler der Referenz), und unter 640px sind kleine Bedienelemente 40px hoch.
