...
1. **Styles.** `spec/components/bundle.css` ist die Referenz. Jeder Umsetzer übernimmt die Abschnitte seiner Bausteine wörtlich in seine Style-Datei. Literale Werte, die dort stehen (Schriftgrößen 12 bis 56px, 1px/2px-Linien, 6px-Punkt, 16px-Spinner usw.), sind damit abgenommen. Jeder neue Wert außerhalb der Tokens ist ein Fund, außer er ist im Abschlussbericht begründet. Dokumentierte Abweichungen von bundle.css: die Basisregel für font/color nutzt `:where()` (Spezifitätsfehler der Referenz); unter 640px sind kleine Bedienelemente 40px hoch; die beiden Link-Basisregeln heißen `.z-root :where(a)` und `.z-root :where(a):hover`, aus demselben Spezifitätsgrund (bei 0,1,1 schlagen sie jede App-Klasse auf einem Link, in der ersten echten Einbindung wurde daraus ein roter Skip-Link auf roter Fläche mit 1,29:1); und die Seitengröße steht in `.z-root:where(:not(html))` statt in `.z-root`, weil die Referenzregel für die Seite gedacht ist und auf `<html>` sonst die rem-Basis der ganzen, noch nicht umgestellten App von 16px auf 14px zieht. Damit setzt die Bibliothek auf `<html>` gar nichts mehr, und die eigene Regel der Anwendung (`html { font-size: var(--base-font-size) }` für eine Einstellung des Nutzers) gewinnt dort bei jeder Spezifität und in jeder Reihenfolge; `:where()` wiegt (0,0,0), die Regel bleibt also bei (0,1,0), genau dem Gewicht von `.z-root`. Der frühere Versuch `html.z-root { font-size: 100% }` wog (0,1,1) und schlug ebendiese App-Regel (gemessen: 14px und 18px ergaben beide 16px); `html:where(.z-root)` geht auch nicht, das ist (0,0,1) und verliert gegen `.z-root` (0,1,0). `body.z-root` trägt die Klasse selbst und behält 14px/20px; die Bausteine der Bibliothek rechnen in px; das einzige `rem` in ihren Stylesheets ist das `1rem` von `.z-legacy`, das einer noch nicht umgestellten Seite die Größe des Besuchers zurückgibt. Dazu kommt `.z-legacy` für einen noch nicht umgestellten Teilbaum (`docs/legacy.md`): die Klasse setzt `font-size: 1rem`, `line-height: normal` und `-webkit-font-smoothing: auto`, und die sechs Basisregeln für nackte Elemente (`.z-root *`, `:where(button, input, select, textarea)`, `:where(a)` samt Hover, die Unterstreichung im Fließtext, `:focus-visible`) stehen je als zwei Selektoren da, `.z-root X:not(:where(.z-legacy, .z-legacy *))` und `:where(.z-legacy) .z-root X`, beide `:where()` mit Spezifität (0,0,0): der erste nimmt den Träger der Klasse und alles darin aus, der zweite schaltet die Regel in einem `z-root` innerhalb von `.z-legacy` wieder ein. Abgenommener Preis: +14 bis +18 % auf einen erzwungenen vollständigen Style-Recalc (4,2 auf 4,9 ms bei 4130 Elementen), allein durch den Ausschluss an der Universalregel. Schrift, Farbe, Hintergrund und `color-scheme` wiederholt die Anwendung selbst. Unter 900px ist die Kopfzeile eine Zeile aus Logo, rechtem Bereich und Menü-Button (`gap: space-3`, `order: 1` am Button, der rechte Bereich bricht in sich um), und ein Bild in `[zBrand]` ist ein Block. `.z-footer` trägt zusätzlich `background: var(--bg)`, wie `.z-header`: die Referenz hat dort keinen, weil ihre Seite ohnehin auf `--bg` steht, doch in einer migrierenden Anwendung liegt unter `body.z-root` teils noch eine helle Altfläche, auf der die Footer-Schrift sonst unlesbar würde. Auf einer migrierten Seite ändert sich nichts, weil die Farbe die der Seite ist. Gemessen mit einer hellen `.z-legacy`-Insel auf `/muster/legacy` (`e2e/legacy.spec.ts`): `.z-footer__base` und seine Links tragen bereits eigene `color`-Regeln (`text-subtle`, `text-muted`), erben also nichts von der Altfläche, weshalb kein zusätzliches `color` nötig war.
