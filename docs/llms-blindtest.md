...
| 4   | `&nbsp;` inside TypeScript object literals of examples (`[total]="{ value: '7,74&nbsp;€' }"`); ` ` is never mentioned. Renders literally.                                                                            | 1, latent for all | JSDoc of `marketing/price-summary.ts`, `configurator/config.ts`; money rule in the generator       |
