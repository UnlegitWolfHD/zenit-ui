...
/**
 * Die Prüfungen, die jede Seite des Design-Systems bestehen muss: axe, kein
 * waagerechtes Scrollen, die Stil-Regeln im berechneten Style, ein sichtbarer
 * Fokus-Ring auf jedem Tab-Stopp und Klickziele ab 40px.
 *
 * Sie stehen hier, weil sie für die Demo-App (`demo.spec.ts`) und für die
 * Beispiel-App (`beispiel.spec.ts`) wortgleich gelten. Jede Funktion prüft die
 * Seite, die gerade offen ist; geöffnet wird mit {@link seiteOeffnen}.
 */

/** Die 7 Größen aus CLAUDE.md, "Typografie". */
export const SCHRIFTGROESSEN = [12, 14, 16, 20, 28, 40, 56];
/** Die 3 Schriften aus tokens.css. Material Icons ist Icon-Schrift, kein Textstil. */
export const TEXTSCHRIFTEN = ['Inter', 'Space Grotesk', 'JetBrains Mono'];
/** radius-sm, radius-md, radius-full; 50 % als gleichwertige Schreibweise. */
export const RADIEN = ['0px', '0', '4px', '8px', '9999px', '50%'];

/** Öffnet eine Route in der gewünschten Breite und wartet auf Titel und Schriften. */
export async function seiteOeffnen(page: Page, route: string, breite: number, hoehe = 900) {
...
/** axe mit den Tags wcag2a, wcag2aa und wcag21aa: keine Verstöße. */
export async function pruefeAxe(page: Page, name: string) {
  ...
  expect(lesbar, `axe-Verstöße auf ${name}`).toEqual([]);
}

/** Die offene Seite scrollt nicht waagerecht. Vorher in 360px öffnen. */
export async function pruefeKeinScrollen(page: Page, name: string) {
  ...
  expect(
    masse.scrollWidth,
    `${name} scrollt bei 360px waagerecht (${masse.scrollWidth} > ${masse.clientWidth})`,
  ).toBeLessThanOrEqual(masse.clientWidth);
}

/** Keine Verläufe, kein backdrop-filter, kein text-shadow, nur die erlaubten Radien, Schriften und Größen. */
export async function pruefeStilRegeln(page: Page, name: string) {
  ...
  expect.soft(bericht.verlauf, `${name}: Verläufe`).toEqual([]);
  expect.soft(bericht.backdrop, `${name}: backdrop-filter`).toEqual([]);
  expect.soft(bericht.schatten, `${name}: text-shadow`).toEqual([]);
  expect.soft(bericht.radius, `${name}: border-radius außerhalb 0/4/8/9999px`).toEqual([]);
  expect.soft(bericht.schriftFunde, `${name}: fremde Schriftfamilie`).toEqual([]);
  expect
    .soft(bericht.groessenFunde, `${name}: Schriftgröße außerhalb 12/14/16/20/28/40/56px`)
    .toEqual([]);
  expect
    .soft(bericht.textschriften.length, `${name}: Textschriften ${bericht.textschriften}`)
    .toBeLessThanOrEqual(3);
}

/** Jeder Tab-Stopp zeigt einen 2px-Ring, am Element selbst oder an seinem Nachbarn. */
export async function pruefeFokusRing(page: Page, name: string) {
  ...
  for (let schritt = 0; schritt < 300; schritt++) {
    ...
    funde.push(
      `${info.name}: outline "${info.outline}"` +
        (info.nachbarRing
          ? ` (Nachbar-Ring ${info.nachbarRing}, aber Element ist sichtbar)`
          : ' (kein Ring am Element und keiner am Nachbarn)'),
    );
  }
  ...
  expect(gesehen.size, `${name} hat keinen Tab-Stopp`).toBeGreaterThan(0);
  expect(funde, `${name}: Tab-Stopps ohne sichtbaren 2px-Fokus-Ring`).toEqual([]);
}

/** Klickziele sind mindestens 40px hoch. Vorher in 375px öffnen. */
export async function pruefeKlickziele(page: Page, name: string) {
  const ziele = await page.evaluate(() => {
    ...
    for (const el of Array.from(auswahl)) {
      ...
      if ((st.clipPath !== 'none' || st.clip !== 'auto') && eigen.width <= 2 && eigen.height <= 2)
        continue;
  ...
  expect(ziele.funde, `${name}: Klickziele unter 40px Höhe bei 375px`).toEqual([]);
}
