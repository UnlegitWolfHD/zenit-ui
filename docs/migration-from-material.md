...
1. **`z-root` on `<html>` and `<body>`.** Both, as the package README prescribes: overlays hang on `<body>` and need the scope, and on `<html>` the class sets no `font-size` and no `line-height`, so the rem base stays with the visitor and with your own `html { font-size: … }`.
